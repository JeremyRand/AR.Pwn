...
void drone_remote_control(float max_tilt, float max_yaw_vel, float max_z_vel)
{
	...
	while(1)
	{
		// Added because we're not using threads in this example
		blob_track();
		...
		printf("Cam1 Chan0 Blob0: x=%d y=%d w=%d h=%d, ms=%d\n", udp_output[1*4*4].x, udp_output[1*4*4].y, udp_output[1*4*4].w, udp_output[1*4*4].h, blob_last_timestamp);

		// Add this back when we're using threads
		//msleep(50);
	}
}
